...
// Qwen2MoE forward pass.
// Reference: llama.cpp llama-model.cpp:8958-9115 (llm_build_qwen2moe).
//
// Architecture: Variable layers/dims, with shared experts and expert weight normalization.
// Has biases on attention projections (unlike Mixtral/Grok).
// Normalizes expert weights (sum normalization per token).
// Includes shared experts (always active) in addition to routed experts.
//
// Note: This implementation handles the routed experts only.
// Shared experts would require additional FFN path (not yet implemented).
...
module @qwen2moe {
  ...
  util.func public @forward(
    ...
    // Qwen2MoE has biases on attention.
    %true = arith.constant true

    // Qwen2MoE normalizes expert weights.
    %normalize_weights = arith.constant true

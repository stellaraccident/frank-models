...
// Full Attention Block: QKV projection, RoPE, GQA attention, output projection.
// Composes position_components.rope and attention_components.attention_gqa.
//
// Usage:
//   %output = call @attention_block(%input, %positions, %wq, %wk, %wv, %wo,
//       %bq, %bk, %bv, %bo, %use_bias, %n_head, %n_head_kv, %n_embd,
//       %rope_freq_base, %rope_freq_scale)
//       : (tensor<?x?x?xf32>, tensor<?x?xi64>, ...) -> tensor<?x?x?xf32>

module @attention_block_components {

  // External dependencies resolved by iree-link.
  util.func private @position_components.rope(
      tensor<?x?x?x?xf32>, tensor<?x?xi64>, f32, f32
  ) -> tensor<?x?x?x?xf32>
  ...
  util.func public @attention_block(
    ...
    // Expand weights from [K, N] to [batch, K, N] for batch_matmul.
    ...
    // Output projection - broadcast weight to 3D.
    ...
    // Conditionally add output bias if enabled.
